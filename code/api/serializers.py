...
class LoginSerializer(serializers.Serializer):
    ...
    def validate_login(self,value):
        ...
        if requests.get(url_index).status_code != 200:
            print(url_index)
            raise serializers.ValidationError(f"URL incorrecte - le site doit être accessible à l'adresse:{base_url}{value}{site}index.html")
        else:
